...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan the qr code di hp satunya lagi'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `hey kak @${num.split('@')[0]} Hiks Titip Gorengan Yah Btw Gausa Kembali Lagi... Kembali = kick 😋`
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '[TUNGGU] HIKS sedang Memperoses🍥Orang sabar Di Sayang Dajal!',
				asik: '[TUNGGU] HIKS sedang Memperoses🍥Orang sabar Di Sayang Dajal!',
				...
				only: {
					...
					mod: '[❗] PERINTAH INI KHUSUS USER *MOD HIKS BOT*',
					...
					userB: `──「 DAFTAR 」──\nHalo Kak !\nMaaf Kak Kamu belum Terdaftar Sebagai Teman Hiks Daftar Untuk Berteman Dengan Ici bot dengam cara, \n\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar Hiks|17\n\n──「 HIKS BOT 」──`,
			...
			switch(command) {
				...
				if (!isUser) return reply(mess.only.userB)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT HIKS ✨* 」\n*│+wa.me/6281368646011*╰──────*「 *HIKS* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  HIKS BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/PcQ6tsB/79ac87b9358c.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6281368646011*╰──────*「 * HIKS* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  HIKS BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
	...
	break
                ...
                case 'irii':
			client.sendPtt(from, './zound/iri2.mp3', id)
			break
		case 'iri':
			client.sendPtt(from, './zound/iri.mp3', {quoted: mek, ptt:true})
			break
                case 'baka':
            case 'onichan':
            case 'bodoh':
                client.sendMessage(dari, './zound/baka.mp3', audio/mp3, {quoted: mek, ptt:true})
                break
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam wr.wb${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*PULSA :* _0877-1474-5448_\n*Saweria :* _https://saweria.co/PIDlDl\n*Dana :* _0877-1474-5440_', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
 ...
 if (isBanned) return reply(mess.only.benned)    
                ...
                client.sendMessage(hasil, image, mek, '\`\`\`HIKSJOKES\`\`\`')
				break
					case 'kata':
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (!isOwner) return reply(mess.only.ownerB)
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner HIKS!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin HIKS!')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
